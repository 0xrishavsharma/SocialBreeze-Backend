    ...
    : describe("BuyMeACoffee", () => {
          ...
          describe("fund", () => {
              ...
              it("Fails if the mapping isn't updated correctly", async () => {
                  ...
                  const response = await buyMeACoffee.getAddressToAmountFunded(
                      deployer
                  )
          ...
          describe("withdraw function", () => {
              beforeEach(async () => {
                  const fundBuyMeACoffee = await buyMeACoffee.fund({
                      value: fundValue,
                  })
              })
              ...
              it("Withdraw ETH from single funder", async () => {
                  // Arrange
                  const startingBuyMeACoffeeBalance =
                      await buyMeACoffee.provider.getBalance(
                          buyMeACoffee.address
                      )
                  const startingDeployerBalance =
                      await buyMeACoffee.provider.getBalance(deployer)
                  ...
                  const endingBuyMeACoffeeBalance =
                      await buyMeACoffee.provider.getBalance(
                          buyMeACoffee.address
                      )
              ...
              it("Allows us to withdraw when there are multiple funders", async () => {
                  const accounts = await ethers.getSigners()
                  for (i = 1; i < 6; i++) {
                      const buyMeACoffeeConnectedContract =
                          await buyMeACoffee.connect(accounts[i])
                      await buyMeACoffee.fund({ value: fundValue })
                  }
                  ...
                  for (i = 1; i < 6; i++) {
                      assert.equal(
                          await buyMeACoffee.getAddressToAmountFunded(
                              accounts[i].address
                          ),
                          0
                      )
              ...
              it("Allows only owner to withdraw", async () => {
                  ...
                  const attackerConnectedContract = await buyMeACoffee.connect(
                      attacker
                  )
                  await expect(
                      attackerConnectedContract.withdraw()
                  ).to.be.revertedWith("BuyMeACoffee__NotOwner")
              })
              ...
              it("cheaperWithdraw ETH from single funder", async () => {
                  ...
                  const withdrawingBalance =
                      await buyMeACoffee.cheaperWithdraw()
                  const withdrawalReceipt = await withdrawingBalance.wait(1)
